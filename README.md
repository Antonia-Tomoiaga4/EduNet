...
- Java Development Kit compatible with the Spring Boot version
...
Create a dedicated database user or configure the application with an existing local PostgreSQL account.

### 3. Configure the backend

The backend requires database and JWT configuration. Keep secrets outside Git and use environment variables or a local profile.

Typical values include:

```properties
spring.datasource.url=jdbc:postgresql://localhost:5432/edunet
...
app.jwt.secret=<strong-random-secret>
app.upload.directory=<local-upload-directory>
```

Property names should match the configuration classes used by the project.
...
When no Maven Wrapper is present:
...
For projects configured with Create React App, use:
...
Use the frontend URL displayed by the development server. The backend must be running and the frontend API base URL must point to the Spring Boot server.
